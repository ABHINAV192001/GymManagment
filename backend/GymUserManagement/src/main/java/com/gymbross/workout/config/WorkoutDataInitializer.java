        ...
        try {
            ...
            try {
                jdbcTemplate.execute("""
                    ...
                    SELECT 
                        v.id::uuid, v.workout_id::uuid, e.id, v.sets, v.reps, v.time
                    FROM (VALUES
                        ('c1111111-0000-0000-0000-000000000051', 'b1111111-0000-0000-0000-000000000005', 4, 15, 45),
                        ('c1111111-0000-0000-0000-000000000052', 'b1111111-0000-0000-0000-000000000005', 4, 15, 45)
                    ) AS v(id, workout_id, sets, reps, time)
                    CROSS JOIN (SELECT id FROM exercises LIMIT 1) e
                    ON CONFLICT (id) DO NOTHING;
                """);
            } catch (Exception ex) {
                log.warn("WorkoutDataInitializer workout_exercises mapping skipped or already seeded: {}", ex.getMessage());
            }

            log.info("12 Preset Workout Splits & Exercise mappings initialized successfully in PostgreSQL.");
        } catch (Exception e) {
            log.warn("WorkoutDataInitializer completed with warnings: {}", e.getMessage());
        }
    }
}
