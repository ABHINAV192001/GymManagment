...
public class FitnessSessionService {
    ...
    public FitnessSession createSession(FitnessSession session) {
        ...
        try {
            // Notify recipients
            List<Long> branchIds = Arrays.stream(session.getBranchIds().split(","))
                    .map(String::trim)
                    .map(Long::parseLong)
                    .collect(Collectors.toList());

            List<Role> roles = Arrays.stream(session.getRecipientRoles().split(","))
                    .map(String::trim)
                    .map(Role::valueOf)
                    .collect(Collectors.toList());
    ...
    public String checkVoteStatus(Long sessionId, String username) {
        return sessionVoteRepository.findBySessionIdAndUsername(sessionId, username)
